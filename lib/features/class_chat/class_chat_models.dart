...
/// One row of `public.class_messages`. Either the class teacher or one
/// of the enrolled students. The `senderName` is best-effort hydrated
/// from `public.profiles` and may be null if RLS hides the row.
class ChatMessage {
  const ChatMessage({
    required this.id,
...
/// Compact summary row for "my classes" — class name + teacher name +
/// number of unseen messages + a flag for "teacher is online right now".
class StudentClassSummary {
  ...
  final bool isTeacherOnline;

  StudentClassSummary copyWith({
    int? unreadCount,
    DateTime? lastActivityAt,
    bool? isTeacherOnline,
  }) {
    return StudentClassSummary(
      ...
      isTeacherOnline: isTeacherOnline ?? this.isTeacherOnline,
    );
  }
}
